...
lse <- simMult(pr = p, libSize = 1000, tree = tr, 
               minTip.A = 20, 
               ratio = 1/3, scenario = "S3",
               nSam = c(10, 10), pct = 0.7)
...
treeFig <- viewSim(lse, branch.length = "none",
                   layout = "circular", zoom_scale = 1, size = 1.5)
treeFig
```
...
```{r}
treeFig +
  geom_point2(aes(subset = (node %in% loc)), color = "red", size = 3)
...
anim_save("output/pk_S3.gif", pk_S3, height = 800, width = 800)
```
